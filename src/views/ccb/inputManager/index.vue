...
<!--          </el-col>-->
    ...
    <el-table v-loading="loading" :data="list" :stripe="true" :show-overflow-tooltip="true">
      ...
      <!--      <el-table-column label="所属领域" align="center" prop="domain" v-if="columns[23].visible"/>-->
      <!--      <el-table-column label="所属产品" align="center" prop="product" v-if="columns[24].visible"/>-->
      <!--      <el-table-column label="所属子产品" align="center" prop="subProduct" width="100" v-if="columns[25].visible"/>-->
      <!--      <el-table-column label="PAM组件1" align="center" prop="pamComponent1" width="100" v-if="columns[26].visible"/>-->
      <!--      <el-table-column label="PAM组件2" align="center" prop="pamComponent2" width="100" v-if="columns[27].visible"/>-->
      <!--      <el-table-column label="PAM组件3" align="center" prop="pamComponent3" width="100" v-if="columns[28].visible"/>-->
      <el-table-column label="规格" align="center" prop="specification" v-if="columns[20].visible"/>
      <el-table-column label="厂商" align="center" prop="vendor" width="100" v-if="columns[21].visible"/>
      <el-table-column label="设备高度" align="center" prop="deviceHeight" v-if="columns[22].visible"/>
      <el-table-column label="功率（千瓦）" align="center" prop="powerConsumption" width="100" v-if="columns[23].visible"/>
      <el-table-column label="备注" align="center" prop="remarks" v-if="columns[24].visible"/>
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width" width="120" v-if="columns[25].visible" fixed="right">
...
import inputDialogForm from './components/inputDialogForm.vue'
...
export default {
  ...
  data () {
    return {
      ...
      columns: [
        { key: 0, label: '序列号', visible: true, disable: true },
        { key: 1, label: '名称', visible: true, disable: true },
        { key: 2, label: '配置概要', visible: false },
        { key: 3, label: '合同号', visible: false },
        { key: 4, label: '事项编号', visible: false },
        { key: 5, label: '技术栈', visible: true, disable: true },
        { key: 6, label: '地域', visible: false },
        { key: 7, label: '可用区', visible: false },
        { key: 8, label: '开始使用日期', visible: true },
        { key: 9, label: '折旧/摊销开始日期', visible: true, disable: true },
        { key: 10, label: '资产分类', visible: true, disable: true },
        { key: 11, label: '合同单价（元）', visible: false },
        { key: 12, label: '单价（元）', visible: true },
        { key: 13, label: '保有量', visible: true },
        { key: 14, label: '保有量单位', visible: true },
        { key: 15, label: '总价（元）', visible: true },
        { key: 16, label: '折旧/摊销年限（月）', visible: true },
        { key: 17, label: '残值率', visible: true },
        { key: 18, label: '月成本（元）', visible: true, disable: true },
        { key: 19, label: '资产状态', visible: true, disable: true },
        // { key: 23, label: '所属领域', visible: true, disable: true },
        // { key: 24, label: '所属产品', visible: true, disable: true },
        // { key: 25, label: '所属子产品', visible: true, disable: true },
        // { key: 26, label: 'PAM组件1', visible: false },
        // { key: 27, label: 'PAM组件2', visible: false },
        // { key: 28, label: 'PAM组件3', visible: false },
        { key: 20, label: '规格', visible: false },
        { key: 21, label: '厂商', visible: false },
        { key: 22, label: '设备高度', visible: false },
        { key: 23, label: '功率（千瓦）', visible: false },
        { key: 24, label: '备注', visible: false },
        { key: 25, label: '操作', visible: true, disable: true }
      ],
      // 导入参数
      upload: {
        // 是否显示弹出层（用户导入）
        ...
        isUploading: false,
        // 上传的地址
        url: process.env.VUE_APP_BASE_API + '/admin-api/system/unused-costs/import/hardware',
        dateList: [
          { label: '硬件设备类', value: '硬件设备类', url: '/admin-api/system/unused-costs/import/hardware' },
          { label: '软件产品类', value: '软件产品类', url: '/admin-api/system/unused-costs/import/software' },
          { label: '管理费用类', value: '管理费用类', url: '/admin-api/system/unused-costs/import/managementCost' },
          { label: '能耗电费类', value: '能耗电费类', url: '/admin-api/system/unused-costs/import/energy' },
          { label: '机柜配套类', value: '机柜配套类', url: '/admin-api/system/unused-costs/import/cabinet' },
          { label: '机房环境类', value: '机房环境类', url: '/admin-api/system/unused-costs/import/serverRoomEnv' }
        ],
        typeValue: '硬件设备类'
      }
    }
  ...
  methods: {
    ...
    async getList () {
      try {
        this.loading = true
        const res = await inputApi.getUnusedCostsPage(this.queryParams)
    ...
    async handleDelete (row) {
      ...
      await this.$modal.confirm('是否确认删除战略成本投入编号为"' + id + '"的数据项?')
      try {
        await inputApi.deleteUnusedCosts(id)
        await this.getList()
    ...
    async handleDeleteAll () {
      ...
      try {
        await inputApi.deleteAllUnusedCosts()
        await this.getList()
        this.$modal.msgSuccess('删除成功')
      } catch {}
    },
    /** 导出按钮操作 */
    async handleExport () {
      await this.$modal.confirm('是否确认导出所有战略成本投入数据项?')
      try {
        this.exportLoading = true
        const res = await inputApi.exportUnusedCostsExcel(this.queryParams)
        this.$download.excel(res, '战略成本投入管理.xls')
      } catch {
      } finally {
    ...
    /** 导入操作 */
    async handleImport () {
      this.upload.url = process.env.VUE_APP_BASE_API + '/admin-api/system/unused-costs/import/hardware'
      if (this.$refs.upload) {
        this.$refs.upload.clearFiles()
      }
      this.upload.typeValue = '硬件设备类'
      this.upload.title = '战略成本投入管理'
    ...
    importTemplate (type, title) {
      inputApi.importTemplate(type).then(response => {
